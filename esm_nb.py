...
train_dataset = EpitopeRawDataset('Positive_train.txt', 'Negative_train.txt', tokenizer=tokenizer, data_dir='./data')
test_dataset = EpitopeRawDataset('Positive_test.txt', 'Negative_test.txt', tokenizer=tokenizer, data_dir='./data')
...
def get_representations(dataset, pretrained_model, alphabet, batch_size=128):
    ...
    for i, (tokens, labels, seqs) in enumerate(progress_bar):
        ...
        with torch.no_grad():
            results = pretrained_model(batch_tokens, repr_layers=[33], return_contacts=True)
...
# %%
figure = plot_roc_curve(train_dataset.labels, lr.predict_proba(train_representations)[:,1], test_dataset.labels, lr.predict_proba(test_representations)[:,1])
