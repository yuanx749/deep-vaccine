...
def plot_roc_curve(train_labels, train_probs, test_labels, test_probs):
    ...
    axes.plot(
        train_fpr,
        train_tpr,
        label="ROC AUC (training) = {:.2f}".format(metrics.auc(train_fpr, train_tpr)),
    )
    axes.plot(
        test_fpr,
        test_tpr,
        label="ROC AUC (test) = {:.2f}".format(metrics.auc(test_fpr, test_tpr)),
    )
    axes.set_xlabel("FPR")
    axes.set_ylabel("TPR")
    axes.legend()
...
def plot_representations(representations, labels):
    ...
    axes.scatter(
        X[:, 0], X[:, 1], c=labels, cmap="tab10", vmax=10, alpha=0.7, linewidths=0
    )
    axes.set_xticks([])
    axes.set_yticks([])
    return fig
